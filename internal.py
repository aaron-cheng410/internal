...
SEED_INPUTS_PATH = '/Users/acheng/Downloads/openbids_testinput.xlsx'
...
def ensure_archived_tables():
    ...
    def make_like(source_table: str, archive_table: str):
        ...
        if insp.has_table(archive_table):
            # Ensure archive has any new columns from source (e.g., Lead Score)
            if src_cols:
                ...
                if to_add:
                    with engine.begin() as conn:
                        ...
                        # archived_at column might already exist; add if missing
                        if "archived_at" not in existing:
                            conn.execute(text(f'ALTER TABLE {archive_table} ADD COLUMN archived_at TIMESTAMP'))
            return

        # Create new archive table shaped like source + archived_at
        if src_cols:
            ...
            ddl = "CREATE TABLE {archive_table} (archived_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)"
        with engine.begin() as conn:
            conn.execute(text(ddl))

    # Results archive unchanged
    make_like("results", "archived_results")
    # General archive should mirror general_internal now
    make_like("general_internal", "archived_general")
...
def archive_general_rows(rows_df: pd.DataFrame):
    """Move rows from general_internal -> archived_general (by Project Name, Article Link)."""
    if rows_df.empty:
        return 0
    moved = 0
    with engine.begin() as conn:
        ...
        for _, r in rows_df.iterrows():
            conn.execute(
                text('DELETE FROM general_internal WHERE "Project Name"=:pn AND "Article Link"=:al'),
